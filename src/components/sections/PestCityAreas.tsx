...
interface PestCityAreasProps {
  data: PestCityPageData;
}

export function PestCityAreas({ data }: PestCityAreasProps) {
  return (
    <section
      className="py-20 lg:py-28 bg-card"
      aria-labelledby="pest-city-areas-heading"
    >
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <ScrollReveal className="text-center mb-14">
          ...
          <h2
            id="pest-city-areas-heading"
            className="font-heading text-3xl sm:text-4xl text-foreground"
          >
            {data.pestName} Across {data.cityName}
          </h2>
          <p className="mt-4 text-muted-foreground max-w-2xl mx-auto">
            Our technicians provide {data.pestName.toLowerCase()} services to
            every corner of {data.cityName}. Here are the neighbourhoods we
            cover.
          </p>
        </ScrollReveal>
